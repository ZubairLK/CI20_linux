...
dm9000_probe(struct platform_device *pdev)
{
	...
	power = regulator_get(dev, "vcc");
	if (IS_ERR(power)) {
		dev_dbg(dev,"no regulator detected\n");
	} else {
		if (!regulator_is_enabled(power)) {
			ret = regulator_enable(power);
			dev_dbg(dev,"regulator enabled\n");
		}
	}

	reset_gpio = of_get_named_gpio_flags(dev->of_node, "reset-gpio", 0, &flags);
	if (gpio_is_valid(reset_gpio)) {
		ret = devm_gpio_request(dev, reset_gpio, "dm9000_reset");
		if (ret) {
